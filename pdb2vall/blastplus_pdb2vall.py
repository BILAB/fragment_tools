...
from blastplus_numbering_back_to_pdbseqres import blastplus_numbering_back_to_pdbseqres
import ConfigParser
...
config.read(PDB2VALL_PATH + "blastplus_pdb2vall.cfg")
...
idealization_app = ROSETTA_PATH + "main/source/bin/idealize_jd2"
relax_app = ROSETTA_PATH + "main/source/bin/relax"
...
tmp_results = blastplus_numbering_back_to_pdbseqres( pdb )

if not tmp_results:
    stderr.write("ERROR: can't get results from blastplus_numbering_back_to_pdbseqres().\n"); exit()
...
pdb_bfactor_dict           = {}  ## from original pdb "xxxx.pdb"; average of N, CA, C B-factors
...
idealized_pdb_cbxyz_dict     = {}  ## from idealized rosetta output "xxxx_0001_0001.pdb"; the numbering is according to the missing density regions, recorded in disorder_dict.
idealized_pdb_cenxyz_dict     = {}  ## from idealized rosetta output "xxxx_0001_0001.pdb"; the numbering is according to the missing density regions, recorded in disorder_dict.
...
cmd2 = fix_blast2chk + ' ' + pdb
print "pdb2vall.py: running BLAST+ blastplus_run_psiblast.pl to make sequence profile checkpoint..."
...
if exists( pdb + "_0001_0001.pdb"):
    ...
    print cmd
    if options.debug:
        cmd = relax_app + " -in:file:s " + pdb + "_0001.pdb -relax::default_repeats 10 -run:idealize_before_protocol  -optimization:default_max_cycles 20000 -relax:fast -in:file:fullatom -ignore_unrecognized_res -use_input_sc -relax::sequence_file " + relax_sequence_file + " -out:file:output_torsions -chemical:exclude_patches LowerDNA  UpperDNA Cterm_amidation SpecialRotamer VirtualBB ShoveBB VirtualDNAPhosphate VirtualNTerm CTermConnect sc_orbitals pro_hydroxylated_case1 pro_hydroxylated_case2 ser_phosphorylated thr_phosphorylated  tyr_phosphorylated tyr_sulfated lys_dimethylated lys_monomethylated  lys_trimethylated lys_acetylated glu_carboxylated cys_acetylated tyr_diiodinated N_acetylated C_methylamidated MethylatedProteinCterm -chemical:exclude_patches ActeylatedProteinNterm -relax::constrain_relax_to_start_coords -overwrite -database %s" % rosetta_database
    if not options.dry_run:
        system( cmd )
...
if not exists( pdb + "_0001_0001.pdb"):
    ...
    for line in idealized_pdb_cbxyz_lines:
        cbrsd   = line[17:20].strip()
        cbrsn   = int( line[22:26].strip())
        ...
        if fasta_dict[ cbrsn ] == longer_names[ cbrsd ]:
            if fasta_dict[ cbrsn ] != "G":
                cbxcord = line[30:38].strip()
                cbycord = line[38:46].strip()
                cbzcord = line[46:54].strip()
                idealized_pdb_cbxyz_dict[ cbrsn ] = [ cbxcord, cbycord, cbzcord ]
            else:
                cbxcord = 9999.99
                cbycord = 9999.99
                cbzcord = 9999.99
    ...
    idealized_pdb_cenxyz_lines = jump_over_missing_density( fasta_output_fn, fasta_output_fn[:5], "centroid")
    for line in idealized_pdb_cenxyz_lines:
        cenrsd   = line[17:20].strip()
        cenrsn   = int( line[22:26].strip())
        ...
        if fasta_dict[ cenrsn ] == longer_names[ cenrsd ]:
                ...
                cenycord = line[38:46].strip()
                cenzcord = line[46:54].strip()
                idealized_pdb_cenxyz_dict[ cenrsn ] = [ cenxcord, cenycord, cenzcord ]
        else:
            stderr.write("ERROR: pdb2vall.py: for position %s of %s.pdb, xyz rsd from idealized pdb doesn't match the fasta from seqres\n" %( cenrsn, pdb )); exit()
...
if not exists( pdb + ".pdb"):
    stderr.write("ERROR:pdb2vall.py: cannot detected the original pdb - %s.pdb\n" % pdb ); exit()
else:
    missing_density_rsn_list = open( fasta_output_fn, "r").readline().split("missing_density_rsn:")[1:][0].strip().split()
    ...
    while line:
        if str(brsn) in missing_density_rsn_list:
            brsn += 1
            continue

        if line[12:16] == " N  ":
            sumbfac += float(line[60:66])
            ncac[0] += 1
        if line[12:16] == " CA ":
            sumbfac += float(line[60:66])
            ncac[1] += 1
        ...
        if 2 in ncac:
            stderr.write("ERROR:pdb2vall.py: error in calc BFactor - %s.pdb\n" % pdb ); exit()
        if not 0 in ncac:
            pdb_bfactor_dict[brsn] = sumbfac/3
            brsn += 1
            sumbfac = 0.0
            ncac = [0,0,0]
    ...
    if options.debug:
        for index in pdb_bfactor_dict.keys():
            print index, pdb_bfactor_dict[ index ]

# N_ali column
if not exists( pdb + ".2.blast"):
    stderr.write("ERROR:pdb2vall.py: cannot detected - %s_.2.blast file \n" % pdb ); exit()
else:
    file = open(pdb+".2.blast", "r")
    line = file.readline()
    n_alilist = []
    n_ali = 0

    while line:
        if line.startswith("Number of sequences better than"):
            n_alilist = line.split()
            n_ali = n_alilist[-1]
...
for rsn in fasta_dict.keys():
    ...
    if rsn > len( fasta_dict.keys()):
        ...
        vall_out.write("%8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f " %( float(idealized_pdb_xyz_dict[ rsn ][0]), float(idealized_pdb_xyz_dict[ rsn ][1]), float(idealized_pdb_xyz_dict[ rsn ][2]),float(idealized_pdb_cbxyz_dict[ rsn ][0]), float(idealized_pdb_cbxyz_dict[ rsn ][1]), float(idealized_pdb_cbxyz_dict[ rsn ][2]),float(idealized_pdb_cenxyz_dict[ rsn ][0]), float(idealized_pdb_cenxyz_dict[ rsn ][1]), float(idealized_pdb_cenxyz_dict[ rsn ][2])))
        ...
        vall_out.write("%8.3f %8.3f %3d %3d " %( float(dssp_Dict[ rsn ][1]), float(dssp_Dict[ rsn ][2]), int(dssp_Dict[ rsn ][0]), int(n_ali)))
