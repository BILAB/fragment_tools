...
sequence alignment program blastp to find the missing density regions.

the reference is taken from pdb_seqres.txt, and trying to match the pdb from get_pdb_new.py
'''

def blastplus_numbering_back_to_pdbseqres( pdb_target_name ):
    ...
    blastp = ""
    if not exists( blastp ):
        blastp = PDB2VALL_PATH + "../../../../../src/blast/bin/blastp" # Robetta location
    if not exists( blastp ):
        blastp = PDB2VALL_PATH + "../blast/bin/blastp" # fragment_tools location
    if not exists( blastp ):
        blastp = PDB2VALL_PATH + "pdb_scripts/blastp" # pdb2val location
    if not exists( blastp ):
        blastp = "/usr/local/bin/blastp" # system location
    if not exists( blastp ):
        blastp = "/usr/bin/blastp" # system location
    if config.get('pdb2vall', 'blastp'):
        blastp = config.get('pdb2vall', 'blastp')
    ...
    if not seq_from_pdbseqres:
        stderr.write("ERROR: blastplus_numbering_back_to_pdbseqres(): can't find %s in pdb_seqres.txt\n\n" % ( pdb_id.lower()+pdb_chain.upper()))
        return 0
    ...
    if exists( pdb_fn ):
        ...
        stderr.write("ERROR:blastplus_numbering_back_to_pdbseqres(): fails to fetch pdb via get_pdb_new.py\n")
        return 0
        exit()

    ## RUN blastp
    cmd = blastp + " -query " + pdb_fn + ".fasta -subject " + pdb_fn[:5] + ".fasta -seg no"
    blastp_results = popen( cmd ).readlines()
    if not blastp_results:
        stderr.write("ERROR: not able to get results from blastp\n")
        return 0

    tmp_fasta_pdbseqres_blastp_list     = []
    tmp_fasta_from_getpdbpy_blastp_list = []
    fasta_pdbseqres_blastp              = ""
    fasta_from_getpdbpy_blastp          = ""

    for line in blastp_results:
        if "Sbjct" in line:
            fasta_pdbseqres_blastp += line.strip().split()[2]
            tmp_fasta_pdbseqres_blastp_list.append( line.strip() )

        if "Query" in line and not pdb_fn in line:
            fasta_from_getpdbpy_blastp += line.strip().split()[2]
            tmp_fasta_from_getpdbpy_blastp_list.append( line.strip() )

    '''### debug
    print  "fasta_pdbseqres_blastp",     fasta_pdbseqres_blastp
    print  "fasta_from_getpdbpy_blastp", fasta_from_getpdbpy_blastp
    print tmp_fasta_pdbseqres_blastp_list #'''

    numbering_ref_Dict     = {}
    numbering_ref_rev_Dict = {}

    sbjct_starting_number = int( tmp_fasta_pdbseqres_blastp_list[0].split()[1] )
    query_starting_number = int( tmp_fasta_from_getpdbpy_blastp_list[0].split()[1] )
    newrsdnum       = sbjct_starting_number
    count           = 1
    missing_density_rsn_list = []
    for rsd in fasta_from_getpdbpy_blastp:
        if rsd == "-":
    ...
    ## ADDING HEAD AND TAIL THAT THE blastp DIDN'T COVER
    '''debug
                    ...
                    if resnum == numbering_ref_rev_Dict[ missing_rsn_from_dissstxt ]:
                        if atomname in ["CA", "N", "C" ]:
                            ...
                            if bfactor >= 95.0:
                                ...
                                #print "!#$! that"
                                #stderr.write("ERROR:blastplus_numbering_back_to_pdbseqres(): dis_ss.txt messed that up.\n")
                                stderr.write("WARNING:blastplus_numbering_back_to_pdbseqres(): dis_ss.txt messed that up.\n")
...
if __name__ == "__main__":
    ...
    results = blastplus_numbering_back_to_pdbseqres( options.pdb_id )
    if results:
        results = results[ options.request ]
        #print blastplus_numbering_back_to_pdbseqres( options.pdb_id )
        #print Dict
