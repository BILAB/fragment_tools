...
my @psiblast = (
  ...
  "/usr/local/bin/psiblast", # /usr/local location
  "/usr/bin/psiblast" # system location
);
...
my @formatdb = (
    ...
    "/usr/local/bin/makeblastdb", # /usr/local location
    "/usr/bin/makeblastdb" # system location 
);
...
my $check = "$blast.chk";
my $blastcmd = "( $PSIBLAST -in_msa $blast -num_iterations 1 -comp_based_stats 1 -inclusion_ethresh 0.001 -evalue 0.001 -num_alignments 0 -out_pssm $check -db $placeholder  ) >& /dev/null";
print "$blastcmd\n";
